...
class Analytics:
    ...
    # Function to compute class-wise accuracy and contribution
    def compute_classwise_metrics(self, model, data_loader, num_classes=10):
        
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        class_correct = [0] * num_classes
        class_total = [0] * num_classes
        class_contributions = [0] * num_classes
        ...
        with torch.no_grad():
            for images, labels in data_loader:
                images, labels = images.to(device), labels.to(device)
                images = images.squeeze(1)  # Ensure shape is [batch_size, 28, 28]
                if images.shape[1] == 32:  # Indicates the channel dimension is incorrectly set as 32
                    # Permute from [batch_size, height, width, channels] to [batch_size, channels, height, width]
                    images = images.permute(0, 3, 1, 2)
                outputs = model(images)
    ...
    def plot_client_contributions_to_calss_compared_to_others(self, client_idx, test_loader, num_classes=10, num_clients=10):
        ...
        # Call the method to compute the accuracies
        aggregated_remaining_clients_model = self.aggregate_remaining_clients(remaining_clients_models)
        client_class_accuracies, client_class_totals = self.compute_classwise_metrics(client_model, test_loader, num_classes)
        remaining_class_accuracies, remaining_class_totals = self.compute_classwise_metrics(aggregated_remaining_clients_model, test_loader, num_classes)
        global_class_accuracies, global_class_totals = self.compute_classwise_metrics(model_before, test_loader, num_classes)
        ...
        # Plot the accuracies as lines
        ax.plot(index, client_class_accuracies, marker='o', label='Client 0 Model', color='blue', linestyle='-', linewidth=2)
        ax.plot(index, remaining_class_accuracies, marker='x', label='Remaining Clients Model', color='green', linestyle='--', linewidth=2)
        ax.plot(index, global_class_accuracies, marker='s', label='Global Model', color='orange', linestyle='-.', linewidth=2)
